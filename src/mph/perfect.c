...
hashform *form; /* user directives */
{
  ...
  if (form->perfect == NORMAL_HP) {
    ...
    *alen = ((form->hashtype == INT_HT) && *smax > 131072) ? ((uint32_t)1 << (UINT32_BITS - mylog2(*blen))) : /* distinct keys => distinct (A,B) */
    ...
    if ((form->hashtype == INT_HT) && *smax < 32)
      ...
      *blen = ((nkeys <= *smax * 0.6) ? *smax / 16 : (nkeys <= *smax * 0.8) ? *smax / 8 : *smax / 4);
    ...
    switch (mylog2(*smax)) {
      ...
      case 17:
        if (form->speed == FAST_HS) {
          ...
          *alen = ((nkeys <= *smax * (5.0 / 8.0)) ? *smax / 8 : (nkeys <= *smax * (3.0 / 4.0)) ? *smax / 4 : *smax / 2);
...
hashform *form; /* user directives */
{
  ...
  *salt       = 0;
  ...
  for (trysalt = 1;; ++trysalt) {
    ...
    if (rslinit == 2) { /* initkey actually found a perfect hash, not just distinct (a,b) */
      *salt = 1;
      *blen = 0;
      break;
    } else if (rslinit == 0) {
